...
for (const file of files) {
  ...
  for (const link of html.matchAll(/(?:href|src)="([^"]+)"/g)) { const target = link[1]; if (target.startsWith('#') || target.startsWith('//')) continue; const resolved = new URL(target, `https://architecturetokens.org${route}`); if (resolved.origin !== 'https://architecturetokens.org') continue; const pathname = resolved.pathname; const fileTarget = pathname.endsWith('/') ? path.join(dist, pathname.slice(1), 'index.html') : path.join(dist, pathname.slice(1)); if (!fs.existsSync(fileTarget) && !routes.has(normalized(pathname))) throw new Error(`${route}: broken internal link ${target}`); }
}
