...
describe('Architecture Tokens website', () => {
  ...
  it.each(['/docs/', '/docs/cli/', '/docs/reference/schemas/architecture-model/', '/docs/reference/specification/', '/docs/specification/common-tokens/'])('serves documentation route %s', async (path) => { const response = await fetchWebsite(`https://architecturetokens.org${path}`); const body = await response.text(); expect(response.status).toBe(200); expect(body).toContain('<main'); expect(body).toContain('content-security-policy'); expect(body).toMatch(/sha256-[A-Za-z0-9+/=]{20,}/); expect(body).not.toContain('unsafe-inline'); expect(response.headers.get('Content-Security-Policy')).toBeNull(); });
  ...
  it('returns branded 404 with status and a meta CSP', async () => { const response = await fetchWebsite('https://architecturetokens.org/not-a-page'); expect(response.status).toBe(404); const body = await response.text(); expect(body).toContain('Page not found'); expect(body).toContain('http-equiv="content-security-policy"'); expect(response.headers.get('Content-Security-Policy')).toBeNull(); expect(response.headers.get('X-Frame-Options')).toBe('DENY'); });
  it('rejects unknown hosts without reflection', async () => { const response = await fetchWebsite('https://untrusted.example/path'); expect(response.status).toBe(404); expect(await response.text()).toBe('Not Found'); expect(response.headers.get('Content-Security-Policy')).toContain("default-src 'none'"); });
});
