...
describe('Architecture Tokens website', () => {
  ...
  it.each(['/docs/', '/docs/cli/', '/docs/reference/schemas/architecture-model/', '/docs/reference/specification/', '/docs/specification/common-tokens/'])('serves documentation route %s', async (path) => { const response = await fetchWebsite(`https://architecturetokens.org${path}`); const body = await response.text(); expect(response.status).toBe(200); expect(body).toContain('<main'); expect(body).toContain('content-security-policy'); expect(body).toMatch(/sha256-[A-Za-z0-9+/=]{20,}/); expect(body).not.toContain('unsafe-inline'); expect(response.headers.get('Content-Security-Policy')).toContain("default-src 'self'"); });
  ...
  it('returns branded 404 with status and an HTML CSP header', async () => { const response = await fetchWebsite('https://architecturetokens.org/not-a-page'); expect(response.status).toBe(404); expect(await response.text()).toContain('Page not found'); expect(response.headers.get('Content-Security-Policy')).toContain("default-src 'self'"); expect(response.headers.get('X-Frame-Options')).toBe('DENY'); });
